...
$(document).ready(function() {
  ...
  $(".selectbox").parent().css("position", "relative");
});
...
function initSelectBox() {
  $(".selectbox dt span").click(function(e) {
    ...
    $(e.target).parents(".selectbox").css("z-index", "100006");
  });
  $(".selectbox dd p").click(
      function(e) {
        $(e.target).addClass('sel').siblings().removeClass('sel');
        $(e.target).parent().parent().find("dt span.fleft").empty().append($(this).text());
        $(e.target).parent().parent().find("dt span.fleft").attr("title", $(this).text());
        // $(e.target).parent().parent().find("dt
        // input.dis").empty().append($(this).attr("name"));
        ...
        if ($(e.target).parent().parent().find("dt").has("input").length != 0) {
          ...
          $(e.target).parent().parent().find("dt").append("<input type='text' class='undis' value='' />");
          $(e.target).parent().parent().find("dt input.undis").val(text);
          $(e.target).parent().parent().find("dt input.undis").attr("name",
              $(e.target).parent().parent().find("dt span.fleft").attr("name"));
        }
      });
...
function show(cover, id) {
  ...
  var s;
  (s = ua.match(/msie ([\d.]+)/)) ? Sys.ie = s[1] : (s = ua.match(/chrome\/([\d.]+)/)) ? Sys.chrome = s[1] : (s = ua
...
function showHtml(cover, id, url) {
  $("#" + id).html("");
  $.ajax({
    url : url,
    dataType : 'text',
...
function hide(cover, id) {
  ...
  var ua = navigator.userAgent.toLowerCase();
  var s;
  (s = ua.match(/msie ([\d.]+)/)) ? Sys.ie = s[1] : (s = ua.match(/version\/([\d.]+).*safari/)) ? Sys.safari = s[1] : 0;
  if (Sys.ie == "6.0") {
    var n = document.getElementsByTagName("select").length;
    for (var i = 0; i < n; i++) {
