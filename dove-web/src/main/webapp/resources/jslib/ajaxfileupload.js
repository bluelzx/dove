...
jQuery.extend({
  ...
  uploadHttpData : function(r, type) {
    var data = !type;
    if (!type) data = r.responseText;
    if (type == "xml" || type == "XML") data = r.responseXML;
    // data = type == "xml" || data ? r.responseXML : r.responseText;
    // If the type is "script", eval it in global context
    if (type == "script" || type == "SCRIPT") jQuery.globalEval(data);
    // Get the JavaScript object, if JSON is used.
    if (type == "json" || type == "JSON") {
      // //////////Fix the issue that it always throws the error "unexpected
      // token '<'"///////////////
      data = r.responseText;
    ...
    if (type == "html" || type == "HTML") jQuery("<div>").html(data).evalScripts();
    return data;
  },
