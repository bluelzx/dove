...
public class WXBizMsgCrypt {
    ...
    private String encrypt(String randomStr, String text) throws AesException {
        ...
        byte[] randomStrBytes = randomStr.getBytes(PKCS7Encoder.CHARSET);
        byte[] textBytes = text.getBytes(PKCS7Encoder.CHARSET);
        byte[] networkBytesOrder = getNetworkBytesOrder(textBytes.length);
        byte[] appidBytes = appId.getBytes(PKCS7Encoder.CHARSET);
    ...
    private String decrypt(String text) throws AesException {
        ...
        try {
            ...
            xmlContent = new String(Arrays.copyOfRange(bytes, 20, 20 + xmlLength), PKCS7Encoder.CHARSET);
            from_appid = new String(Arrays.copyOfRange(bytes, 20 + xmlLength, bytes.length), PKCS7Encoder.CHARSET);
        } catch (Exception e) {
    ...
    public String encryptMsg(String replyMsg, String timestamp, String nonce) throws AesException {
        ...
        // 生成发送的XML
        String result = XMLParse.buildXml(encrypt, signature, timestamp, nonce);
        return result;
    }
     ...
     * @param postData 密文，对应POST请求的数据（已格式化）
     * 
     * @return 解密后的原文
     * @throws AesException 执行失败，请查看该异常的错误码和具体的错误信息
     */
    public String decryptMsg(String msgSignature, String timestamp, String nonce, String postData) throws AesException {
        // 格式化
        postData = String.format("<xml><ToUserName><![CDATA[toUser]]></ToUserName><Encrypt><![CDATA[%1$s]]></Encrypt></xml>",
                postData);
